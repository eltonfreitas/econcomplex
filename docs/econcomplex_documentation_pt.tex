...
\begin{titlepage}
  ...
  \vspace{0.5cm}
  {\large\bfseries Documenta\c{c}\~{a}o T\'{e}cnica --- Vers\~{a}o 1.0.1}\par
  \vspace{1cm}
  \rule{10cm}{0.8pt}\par
  \vspace{0.8cm}
...
def compute_complexity(
    ...
    *,
    ...
    trim: bool = True,                  # poda unidades degeneradas
    dmin: int = 1, umin: int = 1,       # nucleo (use 2,2 se esparso)
...
\begin{notebox}
  \texttt{compute\_complexity} \'{e} uma fina \textbf{camada de
  orquestra\c{c}\~{a}o}: chama as mesmas fun\c{c}\~{o}es p\'{u}blicas que
  voc\^{e} chamaria na m\~{a}o (\fn{rca}, \fn{mcp}, \fn{diversity},
  \fn{ubiquity}, \fn{eci\_pci}, \fn{proximity}, \fn{relatedness\_density},
  \fn{distance}, \fn{coi}/\fn{cog}) e remonta o resultado em formato
  longo, iterando por per\'{i}odo em pain\'{e}is. \'{E} a porta de entrada
  para o caso comum; para controle fino, use as fun\c{c}\~{o}es de matriz
  diretamente.
\end{notebox}
...
\texttt{trim}            & \texttt{bool}      & Poda unidades degeneradas (padr\~{a}o: True) \\
\texttt{dmin}, \texttt{umin} & \texttt{int}   & Limiares do n\'{u}cleo repassados a \fn{eci\_pci}; use 2 para dados esparsos \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{notebox}
  ...
  \texttt{NaN} (ver \fn{trim\_core}). Para dados subnacionais esparsos,
  passe \texttt{dmin=2, umin=2} para restringir ao n\'{u}cleo conectado.
\end{notebox}
